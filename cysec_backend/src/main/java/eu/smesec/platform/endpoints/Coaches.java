...
import eu.smesec.platform.messages.CoachMsg;
...
public class Coaches {
  ...
  public Response resumeCoach(@PathParam("id") String id) {
    String companyId = context.getAttribute("company").toString();
    try {
      context.setAttribute("fqcn", id);  //todo improve
  ...
  @Produces(MediaType.APPLICATION_JSON)
  //todo audit log
  public Response updateAnswer(@PathParam("id") String id,
    ...
    try {
      ...
      // handle answer
      // TODO: Danger for race-condition for deselect
      synchronized (Coaches.class) {
        ...
        Audit audit = AuditUtils.createAudit(userId, UserAction.MODIFIED, before == null ? "" : before, after == null ? "" : after);
      ...
      String url = next != null
              ...
              ? "/api/rest/resources/" + fqcn.getCoachId() + "/" + library.getId() + "/assets/jsp/summary.jsp"
              : "/app";
  ...
  public Response renderQuestion(@PathParam("id") String id,
