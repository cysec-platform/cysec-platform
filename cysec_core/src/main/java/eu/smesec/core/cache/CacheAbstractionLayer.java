...
public class CacheAbstractionLayer {
    ...
    public static synchronized void init(final Supplier<String> contextPath, final Supplier<ResourceManager> resManager, final Supplier<Config> config) {
        class CacheAbstractionLayerFactory implements Supplier<CacheAbstractionLayer> {
            ...
            public CacheAbstractionLayer get() {
                return cacheAbstractionLayerInstance;
            }
        }

        if (!(instance instanceof CacheAbstractionLayerFactory)) {
            instance = new CacheAbstractionLayerFactory();
        } else {
            throw new IllegalStateException("Already Initialized");
        }
    }

    private static Supplier<CacheAbstractionLayer> instance = new CacheAbstractionLayerHolder();

    /**
     * Temp Placeholder supplier
     */
    private static final class CacheAbstractionLayerHolder implements Supplier<CacheAbstractionLayer> {
        @Override
        public synchronized CacheAbstractionLayer get() {
            if (instance instanceof CacheAbstractionLayerHolder)
     ...
     *
     * @return CacheAbstractionLayer instance
     */
    public static CacheAbstractionLayer getInstance() {
        return instance.get();
    }
     ...
     */
    public void createUser(String companyId, User user) throws CacheException, ElementAlreadyExistsException {
        data.executeOnCompany(
                companyId,
                companyCache -> {
    ...
    public User getUserByToken(String companyId, String token) throws CacheException {
        return readOnUsers(
                ...
                company -> {
                    for (User user : company.getUser()) {
                        for (Token token1 : user.getToken()) {
    ...
    public Question getQuestion(String coachId, String qid) throws CacheException {
        return coaches.read(
                ...
                null,
                coachLib -> {
                    for (Question question : coachLib.getFirst().getQuestions().getQuestion()) {
    ...
    public Library getLibrary(String coachId, String libId) throws CacheException {
        return coaches.read(
                ...
                null,
                coachLib -> {
                    for (Library lib : coachLib.getSecond()) {
     ...
     */
    public byte[] zipCompany(String companyId) throws CacheException {
     ...
     * @throws CacheException              If an error occurred.
     */
    public byte[] createFileResponse(String companyId, Path relative) throws CacheException {
        // cannot use normal output stream, because of jersey StreamingOutput
