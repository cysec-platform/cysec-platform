...
public class DashboardModel {
    ...
    private List<CoachHelper> remaining;
    private List<Recommendation> recommendations;
    private List<Badge> badges;
    private LastSelected lastSelected;
    private Tuple<FQCN, Skills> skills;

    CacheAbstractionLayer cal = CacheAbstractionLayer.getInstance();

    public DashboardModel() {
//        FIXME: Define limit for recommendation to show. Currently 3 and hardcoded
        this.recommendationLimit = 3;
        this.locale = LocaleUtils.fromString(null);
        init();
    }

    public DashboardModel(int recommendationLimit, Locale locale) {
    ...
    private void init() {
        this.companyId = LibCal.getCompany();
        this.remaining = new ArrayList<>();
        this.recommendations = new ArrayList<>();
        this.badges = new ArrayList<>();
        this.lastSelected = null;
        this.skills = new Tuple<>(null, null);
    }
    ...
    public Map<String, Object> getModel() throws CacheException {
        ...
        // why are they in the constructor? Injection?
        ...
        processMetadata(companyAnswers, recommendations, badges, coachHelper);

        List<CoachHelper> coachHelpers = new ArrayList<>();
        coachHelpers.add(coachHelper);
//FIXME lastSelected and Skills are never updated
        ...
        recommendations = trimRecommendations(recommendations);
        badges = trimBadges(badges);


        return createModel(locale, allCoaches, coachHelpers, recommendations, remaining, badges, skills);
    }
    ...
    public Map<String, Object> createModel(Locale locale, Map<String, Questionnaire> allCoaches, List<CoachHelper> coachHelpers, List<Recommendation> recommendations, List<CoachHelper> remaining, List<Badge> badges, Tuple<FQCN, Skills> skills) {
        DashboardMsg msg =
        ...
        if (skills != null && skills.getFirst() != null) {
            model.put(
                    "skills",
    ...
    /**
     * Gets the name of an answer
     *
     * @param fqcn    Fully qualified coach name
     * @param coaches map of coaches
    ...
    private void processMetadata(Answers companyAnswers, List<Recommendation> recommendations, List<Badge> badges,  CoachHelper coachHelper) {
        for (Metadata md : companyAnswers.getMetadata()) {
            ...
            if (mdKey.startsWith(MetadataUtils.MD_RECOMMENDED)) {
                ...
                this.lastSelected = MetadataUtils.fromMd(md, LastSelected.class);
                ...
                this.skills = new Tuple<>(LibCal.FQCN_COMPANY, MetadataUtils.fromMd(md, Skills.class));
            }
        }
    }
    ...
    private List<Recommendation> trimRecommendations(List<Recommendation> recommendations) {
