...
public class PasswordStorageTest {
    ...
    public void staticCryptPasswordCases() throws NoSuchAlgorithmException {
        for( String[] testcase : testTuples) {
            ...
            Assert.assertTrue( "testing password \"" + in[0] + "\" with default hash and random salt for verification",     new CryptPasswordStorage(in[0],null).verify(in[0]));
            Assert.assertTrue( "testing password \"" + in[0] + "\" with predefined hash and without salt for verification", new CryptPasswordStorage(in[0], in[1]).verify(in[0]));
        }
    }
    ...
    public void staticCryptStorageCases() throws NoSuchAlgorithmException {
        for( String[] testcase : testTuples ) {
            String[] in = testcase[0].split(":");
            Assert.assertTrue( "testing password \"" + in[0] + "\" from storage \"" + testcase[1] + "\"", new CryptPasswordStorage( testcase[1] ).verify(in[0]));
            Assert.assertEquals("testing storage \"" + testcase[1] + "\" from storage \"" + testcase[1] + "\"!=\"" + new CryptPasswordStorage(in[0], in[1], new CryptPasswordStorage(testcase[1]).getType()).getPasswordStorage() + "\"", new CryptPasswordStorage(in[0], in[1], new CryptPasswordStorage(testcase[1]).getType()).getPasswordStorage(), testcase[1]);
        }
    }
    ...
    public void emptySaltInCryptPasswordStorage() throws NoSuchAlgorithmException {
        for(String[] testcase : testTuples) {
            ...
            try{
                ...
                Assert.assertTrue( "storage did not successfully verify to password", s.verify( in[0] ) );
                Assert.assertTrue( "storage did not autogenerate salt (length=" + s.getSalt().length() + ")", s.getSalt().length() > 10 );
            } catch( Exception iae ) {
                TestCase.fail( "null salt did raise exception" );
            }
            try{
                new CryptPasswordStorage( in[1], "" ).verify( in[0] );
                TestCase.fail( "empty string salt did not raise exception" );
    ...
    public void randomStorageInCryptPasswordStorage() {
        ...
        for( int i=0; i<10000; i++ ) {
            ...
            try {
                ...
                e.printStackTrace();
                TestCase.fail( "caught unexpeted exception while fuzzing storage" );
            }
        }
    }
